    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-4 flex-wrap">
        <div className="relative flex-1 min-w-[200px]">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-2.5 bg-white dark:bg-dark-800 border border-gray-200 dark:border-dark-700 rounded-xl text-sm text-gray-900 dark:text-white focus:ring-2 focus:ring-primary-500 outline-none"
            placeholder="Buscar gastos..."
          />
        </div>
        <div className="bg-white dark:bg-dark-800 rounded-xl px-4 py-2.5 shadow-sm">
          <span className="text-sm text-gray-500 dark:text-dark-400">Total: </span>
          <span className="text-sm font-bold text-gray-900 dark:text-white">{formatCOP(total)}</span>
        </div>
      </div>
      ...
      <div className="space-y-2">
        {filtered.map(expense => (
          <div key={expense.id} className="bg-white dark:bg-dark-800 rounded-xl p-4 shadow-sm flex items-center justify-between gap-4">
            <div className="flex items-center gap-3 flex-1 min-w-0">
              ...
              <div className="min-w-0">
                ...
                <p className="text-xs text-gray-400 dark:text-dark-500">
                  {expense.category_name || 'Sin categoría'} · {new Date(expense.date + 'T12:00:00').toLocaleDateString('es-CO', { day: 'numeric', month: 'short' })}
                </p>
              </div>
            </div>
            <div className="flex items-center gap-3">
        ...
        {filtered.length === 0 && (
          <div className="text-center py-16 text-gray-400 dark:text-dark-500">
            <p className="text-lg mb-2">Sin gastos</p>
            <p className="text-sm">Agrega tu primer gasto con el botón +</p>
          </div>
        )}
      </div>
    </div>
